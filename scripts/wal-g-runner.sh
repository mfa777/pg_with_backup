...
# Source wal-g environment (generated by walg-env-prepare.sh during container startup)
WALG_ENV_FILE="${WALG_ENV_FILE:-/var/lib/postgresql/.walg_env}"
if [ -f "$WALG_ENV_FILE" ]; then
    source "$WALG_ENV_FILE"
else
    echo "ERROR: wal-g environment file not found at $WALG_ENV_FILE"
    exit 1
fi
