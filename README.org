...
- Built-in PostgreSQL contrib extensions available for =pg_search= (e.g. =pg_trgm=, =unaccent=)
...
*** Enable pg_search-related extensions (optional)

The PostgreSQL image includes =postgresql-contrib= so extensions commonly used by Rails =pg_search= can be enabled manually.

Run once in your target database:
#+begin_src sql
